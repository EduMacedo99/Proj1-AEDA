...
int PontosVenda::save_file(string ficheiro){
	...
	for(unsigned i = 0; i < ptsVenda.size(); i++){
		...
		for(unsigned j = 0; j < ptsVenda.at(i)->getBilhetesO().size(); j++){
		...
		if(ptsVenda.at(i)->eLoja()){
			...
			if(l != NULL){
				...
				for(unsigned k = 0; k < l->getBilhetesA().size(); k++){
					...
					//como saber que tipo de assinante e que tem o bilhete, e como mandar isso para o ficheiro?
		...
		//bilhetes assinatura (se for loja)
